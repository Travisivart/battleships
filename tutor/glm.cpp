 ...
 */
GLvoid
glmDraw(GLMmodel* model, GLuint mode, GLuint type)
{
    static GLuint i;
 ...
 * GLM_FLAT and GLM_SMOOTH should not both be specified.  */
GLuint
glmList(GLMmodel* model, GLuint mode, GLuint type)
{
    GLuint list;

    list = glGenLists(1);
    glNewList(list, GL_COMPILE);
    //glmDraw(model, GLM_MATERIAL, type);
    glmDraw(model, mode, type);
    glEndList();
